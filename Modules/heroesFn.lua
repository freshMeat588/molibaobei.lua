...
function module:autoPoint(charIndex,setting)
  ...
  self:logInfo('加点选的模式：   ', setting);
  local levelUpPoint = Char.GetData(charIndex,CONST.CHAR_升级点)
  self:logInfo('升级点：   ', levelUpPoint);
  ...
  for i=1,5 do
    ...
    --self:logInfo('某属性值：   ', type,point,point*100,Char.GetData(charIndex,type));
  ...
  self:logInfo('目前剩余升级点：   ', CONST.CHAR_升级点,levelUpPoint);
