...
function hunter_show_quest( _index_me, _index_tome, _number)
  --在 Lua 中，tonumber 函数用于将一个值转换为数字类型。而 Field.Get 函数则用于获取一个字段的值。
  --
  --根据您提供的代码，tonumber( Field.Get( _index_tome, "hunter_lasttime_".._number)) 的作用是
  --获取 _index_tome 表中名为 "hunter_lasttime_".._number 的字段的值，并将其转换为数字类型，
  --然后将结果赋值给 lasttime 变量。
  --
  --请注意确保 _index_tome 表和 _number 变量的正确性，以及确保字段 "hunter_lasttime_".._number 存在
  --并包含可以转换为数字的值，否则可能会出现错误。
	local lasttime = tonumber( Field.Get( _index_tome, "hunter_lasttime_".._number));
  --在 Lua 中，os.date 函数用于获取当前日期和时间的格式化字符串。os.time 函数用于获取当前时间的时间戳。
  --
  --根据您提供的代码，os.date("%d", os.time()) 的作用是获取当前时间的日期部分，并将其转换为数字类型。
  --具体来说，"%d" 是日期格式化字符串，用于表示只提取日期部分。然后，tonumber 函数将日期部分的字符串转换为数字类型，
  --将结果赋值给 nowtime 变量。
  --
  --请注意，日期格式化字符串 %d 表示日期的两位数表示（例如：01、02、03），如果您想要其他格式的日期表示，
  --请参考 Lua 的日期格式化字符串文档进行修改。同时，确保您的操作系统和时区设置正确，以获取准确的当前日期和时间。
