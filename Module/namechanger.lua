...
function initchanger()
     if(nameChangerNPC == nil)then
		...
		Char.SetData(nameChangerNPC,%对象_X%,240);
		Char.SetData(nameChangerNPC,%对象_Y%,81);
		Char.SetData(nameChangerNPC,%对象_地图%,1000);
...
function nameChangerTalked(_me,player)
	if(NLG.CanTalk(_me,player)==false)then
		return;
	end
	
	--local freeNameChanged = fieldget(player,"NAMECHANGED");
    local freeNameChanged = 0;
    
    local jinbinum = 0;
    if(casttype ~= 0) then
    	jinbinum = tonumber(Char.ItemNum(player,casttype));
    else
    	jinbinum = tonumber(Char.GetData(player,%对象_金币%));
    end


	if(jinbinum < castnum)then
		NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_关闭%,0,"\\n\\n您好,"..(Char.GetData(player,%对象_原名%)).." 这里是改名服务NPC。\\n\\n我可以帮您改名,每次改名费用是"..castnum.."个"..castname.."。\\n\\n似乎您的"..castname.."数量不足呢!\\n");
		return;
	else
		NLG.ShowWindowTalked(player,_me,%窗口_输入框%,%按钮_确定%,1,"\\n\\n您好,"..(Char.GetData(player,%对象_原名%)).." 这里是改名服务NPC。\\n\\n我可以帮您改名,每次改名费用是"..castnum.."个"..castname.."。\\n\\n您可以直接在下面输入您想要改的名字,然后按下确定按钮。\\n");
		return;
	end
end

function nameChangerWTalked(_me,player,_seqno,_select,_data)
	if(NLG.CanTalk(_me,player)==false)then
		return;
	end
	if(tonumber(_seqno)==1 and tonumber(_select)==%按钮_确定% and _data~="")then
		...
		--local freeNameChanged = fieldget(player,"NAMECHANGED");
		local freeNameChanged=0;
		local level = Char.GetData(player,%对象_等级%);
		
		if(_data == Char.GetData(player,%对象_原名%))then
			NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_关闭%,0,"\\n\\n那个...似乎你要改的名字和现在的名字没有什么变化呢...\\n\\n您在确定下您的新名字吧!\\n\\n");
			return;
		end

		if(jinbinum < castnum)then
			NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_关闭%,0,"\\n\\n您好,"..(Char.GetData(player,%对象_原名%)).." 这里是改名服务。\\n\\n我可以帮您改名,每次改名费用是"..castnum.."个"..castname.."。\\n\\n似乎您的"..castname.."数量不足呢!\\n");
			return;
		end
	    ...
	    if(casttype ~= 0) then
	    	jinbinum = tonumber(Char.DelItem(player,casttype,castnum));
	    else
	    	jinbinum = tonumber(Char.SetData(player,%对象_金币%,jinbinum - castnum));
	    end

		NLG.SystemMessage(player,"失去了"..castnum.."个"..castname.."。");
		local query = "Update tbl_character set `Name` = '".._data.."' Where Cdkey = '"..(Char.GetData(player,%对象_账号%)).."' and Name='"..(Char.GetData(player,%对象_原名%)).."'";
		local ret = SQL.Run(query);
		
		--由于没有添加数据库相关字段,暂时注释.(只影响查询NPC)
		--local query2="INSERT INTO tbl_changenamelog (before_name,after_name) VALUES ('"..(Char.GetData(player,%对象_原名%)).."','".._data.."')";
		--local ret = SQL.Run(query2);
		
		Char.SetData(player,%对象_原名%,_data);
		NLG.UpChar(player);
		
		NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_关闭%,3,"\\n\\n恭喜您,您的名字已经成功的改为了".._data.."哦!\\n\\n");
		return;
	end
...
function nameChangerSelectTalked(_me,player)
	if(NLG.CanTalk(_me,player)==false)then
		return;
	end

	if(os.time() - selectnametime < 60)then
		NLG.SystemMessage(player,"一分钟内只允许查询一次，请稍等。");
		return;
	end
	
    local showtext="\\n\\n  改名记录查询(只显示一周内)：\\n\\n  请单击确定开始查询。";
	selectnametime = os.time();
	NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_确定%,0,showtext);


end

function nameChangerSelectWTalked(_me,player,_seqno,_select,_data)
	if(NLG.CanTalk(_me,player)==false)then
		return;
	end
	...
	NLG.ShowWindowTalked(player,_me,%窗口_信息框%,%按钮_上下取消%,_seqno,showtext);
	return;
	

end

function nameTestTalked(_me,player)
  if(NLG.CanTalk(_me,player)==false)then
    return;
  end
...
function nameTestWTalked(_me,player,_seqno,_select,_data)
  if(NLG.CanTalk(_me,player)==false)then
    return;
  end
  if(tonumber(_seqno)==1 and tonumber(_select)==%按钮_确定% and _data~="")then
