...
local NpcMapPos = {0, 1000, 238, 83}		--Npc地图坐标
...
function GetGold(playerIndex)
  local goldData = Char.GetData(playerIndex, %对象_金币%);
  local gold = tonumber(goldData);
  return gold;
end
...
function initTest()
  ...
  if(练级匹配NPC == nil)then
    练级匹配NPC = NL.CreateNpc("lua/Module/EasyWarp.lua", "NameChangerandselectNpc_Init");
    Char.SetData(练级匹配NPC,%对象_形象%,231011);
  Char.SetData(练级匹配NPC,%对象_原形%,231011);
  Char.SetData(练级匹配NPC,%对象_X%,238);
  Char.SetData(练级匹配NPC,%对象_Y%,83);
  Char.SetData(练级匹配NPC,%对象_地图%,1000);
  Char.SetData(练级匹配NPC,%对象_方向%,4);
  Char.SetData(练级匹配NPC,%对象_原名%,"接待员球球");
  NLG.UpChar(练级匹配NPC);
  Char.SetWindowTalkedEvent("lua/Module/EasyWarp.lua","EasyWarpNPC_WindowTalked",练级匹配NPC);
  Char.SetTalkedEvent("lua/Module/EasyWarp.lua","EasyWarpNPC_Talked", 练级匹配NPC);
...
--			NLG.UpChar(player);			
	...
	if tonumber(_Seqno) ==898 then
		...
		if 判断条件登记条件(_PlayerIndex,_MeIndex, 玩家临时记录[_PlayerIndex],ToNumber) == 0 then
	...
	if _Seqno ~=999 and _Seqno < 100 and tonumber(_Data)>0 then --创建匹配练级
		...
		if 玩家临时记录[_PlayerIndex]==2 then
			...
			if 判断条件登记条件(_PlayerIndex,_MeIndex,Level,登记地点) == 0 then
...
function 判断条件登记条件(_PlayerIndex,_MeIndex,Level,ToNumber)
...
function 查看队伍窗口(_PlayerIndex,_Seqno)
    ...
    local Players = {}
        ...
        local 队长索引 = 新玩家匹配列表[_PlayerIndex][i][1]
        local 队长名字 = Char.GetData(队长索引, %对象_原名%)
        local 人数 = 登记人数 .. "/" .. tonumber(新玩家匹配列表[_PlayerIndex][i][2][3])
        buf = buf .. string.format("%8.8s", 队长名字) .. string.format("%12.12s", 地图名称) .. string.format("%8.8s", 要求等级) .. string.format("%8.8s", 传送收费) .. string.format("%8.8s", 人数) .. "\n"
